...
webphone_api.translations = (function () {
  ...
  function Translate(str, secondtry) {
    ...
    if (ret.length < 1) // && str[str.length - 1] == '.'
    {
      ...
      if (
        ret.length < 1 &&
        (typeof secondtry === "undefined" || secondtry === null || !secondtry)
      ) {
        if (str.indexOf("Registered. Credit") == 0)
          return (
            Translate("Registered", true) +
            ". " +
            Translate("Credit", true) +
            str.substr(18)
          );
        if (str.indexOf("Registered") >= 0)
          return str.replace("Registered", Translate("Registered", true));
        if (str.indexOf("Call duration") >= 0)
          return str.replace("Call duration", Translate("Call duration", true));
      }
    }
