...
webphone_api._contactslist = (function () {
  ...
  function onStart(event) {
    try {
      ...
      setTimeout(function () {
        // otherwise keyup will not work, because the element is not focused
      ...
      if (
        ...
        if (count < 2) {
          ...
          //MeasureDialPad();
          if (
            typeof webphone_api._dialpad !== "undefined" &&
            webphone_api._dialpad !== null
          ) {
  ...
  function PopulateList(onlyserver) {
    ...
    try {
      ...
      for (var i = 0; i < ctlistLocal.length; i++) {
        ...
        //ctname = ctname.replace('<', ''); ctname = ctname.replace('>', '');
        //ctname = ctname.replaceAll("<[^>]*>", "");
    ...
    {
        webphone_api.common.PutToDebugLogException(2, "_contactslist: PopulateList Inner ", err);
        //webphone_api.common.PutToDebugLog(5, 'EVENT,listview content '+listview);
    }
    */
      try {
        webphone_api.$("#contactslist_list").append(listview);
      } catch (err) {
        webphone_api.common.PutToDebugLogException(
          2,
          "_contactslist: PopulateList InnerA ",
          err,
        );
        //webphone_api.common.PutToDebugLog(5, 'EVENT,listview content '+listview);
      }
      ...
      try {
        if (webphone_api.$("#contactslist_list").hasClass("ui-listview")) {
          // If it already has the jQuery Mobile listview classes, just refresh
          webphone_api.$("#contactslist_list").listview("refresh");
        } else {
          // If it's not yet initialized (e.g., if the page was dynamically created or if previous operations somehow de-initialized it), initialize it first.
          webphone_api.$("#contactslist_list").listview().listview("refresh");
        }
      } catch (err) {
        webphone_api.common.PutToDebugLogException(
          2,
          "_contactslist: PopulateList InnerB ",
          err,
        );
        //webphone_api.common.PutToDebugLog(5, 'EVENT,listview content '+listview);
      }
      ...
      /*
    webphone_api.$('#contactslist_list').empty();
    if (webphone_api.$('#contactslist_list').hasClass('ui-listview')) {
        // If it already has the jQuery Mobile listview classes, just refresh
        webphone_api.$('#contactslist_list').listview('refresh');
    } else {
        // If it's not yet initialized (e.g., if the page was dynamically created
        // or if previous operations somehow de-initialized it), initialize it first.
        webphone_api.$('#contactslist_list').listview().listview('refresh');
    }
    */
    } catch (err) {
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (
        webphone_api.common.GetParameterInt("serveraddressbook_allowedit", 1) ==
        0
      )
