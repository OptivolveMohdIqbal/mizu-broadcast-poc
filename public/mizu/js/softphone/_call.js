...
webphone_api._call = (function () {
        ...
        try {
          ...
          if (charCode === 13) // enter
          {
            ...
            if (
              ...
              webphone_api.$("#acceptreject_layout").is(":visible")
            ) // acceptreject_layout   check is visible
            {
              webphone_api.$("#btn_accept").click();
              event.preventDefault();
            } else if (
              webphone_api.$("#mline_layout").is(":visible")
            ) // acceptreject_layout   check is visible
            {
          ...
          } else if (charCode === 27 || charCode === 8) // ESC or Backspace
          {
            ...
            ) // acceptreject_layout   check is visible
            {
              ...
              webphone_api.$("#hangup_layout").is(":visible")
            ) // acceptreject_layout   check is visible
            {
  ...
  function onStart(event) {
    try {
      ...
      if (
        ...
        webphone_api.gettelsearchname(callnumber, function (recname) {
          ...
          if (webphone_api.$("#page_call_peer_details").html() != peerdetails) {
            webphone_api.common.PutToDebugLog(
              6,
              "EVENT, caller-id display A: " + peerdetails,
            );
      ...
      if (webphone_api.common.GetConfigInt("brandid", -1) === 18467) {
        webphone_api.$("#contact_image_img").hide();
        webphone_api.$("#contact_image_img").html("");
      }
      ...
      if (calltype === "outgoing") {
        ...
        setTimeout(function () {
          ...
          if (
            ...
            webphone_api.needratingrequest(function (val) {
    ...
    try {
      if (webphone_api.common.isNull(det_in) || det_in.length < 1) {
        if (webphone_api.$("#page_call_peer_details").html().length > 0) {
          webphone_api.common.PutToDebugLog(
            6,
            "EVENT, caller-id display clear A",
          );
      ...
      if (det_in.indexOf(" ") < 0) {
        if (webphone_api.$("#page_call_peer_details").html() != det_in) {
          webphone_api.common.PutToDebugLog(
            6,
            "EVENT, caller-id display B: " + det_in,
          );
      ...
      while (idx < darr.length) {
        if (
          ...
          webphone_api.common.Trim(darr[idx]).length < 1
        ) {
          darr.splice(idx, 1);
        } else {
          idx++;
      ...
      for (var i = 0; i < darr.length; i++) {
        if (
          ...
          webphone_api.common.Trim(darr[i]).length < 1
        ) {
          continue;
        }
...
*/
  ...
  function AcceptCall(callapi) {
    ...
    try {
      ...
      try {
        ...
        webphone_api.common.PutToDebugLogWarning(
          2,
          "_call: AcceptCallInternal " + lastoop.toString(),
          ee,
        );
      }
      lastoop = 5;
      ...
      webphone_api.common.PutToDebugLogException(
        2,
        "_call: AcceptCall " + lastoop.toString(),
        err,
      );
    }
  }
  ...
  function LineCliked(id) {
    try {
      ...
      if (
        webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC
      ) //new behaviour not handling the automute/autohold here for the NS engine
      {
        ...
        var autohold = webphone_api.common.GetParameterInt("autohold", -1);
        ...
        for (var i = 0; i < webphone_api.global.ep.length; i++) {
          ...
          if (webphone_api.global.aline == iline) {
            if (automute >= 4) {
              if (
                item[webphone_api.common.EP_MUTESTATE] == "true"
              ) // if is muted
              {
            ...
            if (automute >= 4) {
              if (
                item[webphone_api.common.EP_MUTESTATE] == "false"
              ) // if not muted
              {
                webphone_api.common.PutToDebugLog(
                  2,
  ...
  function VideoRecall() {
    try {
      ...
      HangupCall();
      setTimeout(function () {
        if (webphone_api.global.isvideorecall < 1)
          webphone_api.global.isvideorecall = 1;
