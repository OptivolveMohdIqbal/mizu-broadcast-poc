...
*/
  ...
  function onCreate(event) {
    ...
    try {
      ...
      webphone_api.$("#phone_number").on("input", function () {
        // input text on change listener
        PhoneInputOnChange();
      });
      ...
      webphone_api
        .$("#dialpad_list")
        .on("taphold", ".ch_anchor", function (event) {
  ...
  function PutNumber(val) {
    try {
      ...
      if (
        webphone_api.$("#phone_number").is(":focus")
...
// set status width so it's uses all space to curr_user
  ...
  function SortRecents() {
    try {
      webphone_api.global.recentlist.sort(function (a, b) {
        // comparator function
  ...
  function SearchContacts(searchval) {
    try {
      if (
        ...
        webphone_api.common.Trim(searchval).length < 1
      ) {
        webphone_api.common.PutToDebugLog(
          2,
  ...
  function RecentMenu(rcid, isrecent, islongclick, popupafterclose) {
    try {
      ...
      if (
        webphone_api.common.GetContactIdFromNumber(rcnumber) >= 0
      ) // if contact exists
      {
  ...
  var MENUITEM_DIALPAD_DEVTEST = "#menuitem_dialpad_devtest";
  ...
  function CreateOptionsMenu(menuId) {
    ...
    try {
      ...
      if (webphone_api.common.GetParameterBool("devtestmenu", false) === true) {
        webphone_api
          .$(menuId)
          .append(
            '<li id="' +
              MENUITEM_DIALPAD_DEVTEST +
              '"><a data-rel="back">DevTest</a></li>',
          )
          .listview("refresh");
      }
  ...
  function MenuItemSelected(itemid) {
    try {
      ...
      webphone_api.$("#dialpad_menu").on("popupafterclose", function (event) {
        ...
        if (
          itemid.indexOf("dialpad_custmenu_") === 0
        ...
        switch (itemid) {
          ...
          case MENUITEM_DIALPAD_FILETRANSFER:
            ...
            break;
          case MENUITEM_DIALPAD_DEVTEST:
            webphone_api.common.DevTest();
            break;
    ...
    try {
      ...
      if (cfr === 1 || cfr === 3) {
        ...
        var template =
          ...
          '<input type="text" id="conferencedtmf_input" name="setting_item" data-theme="a" dir="auto" autocapitalize="off" value="' +
          number +
          '"/>' +
  ...
  function CreditRecharge() {
    try {
      ...
      if (
        webphone_api.common.Trim(ruri).indexOf("*") !== 0
  ...
  function VideoCall(phoneNr) {
    ...
    try {
      ...
      var template =
        ...
        lastDialed +
        '" data-theme="a" dir="auto" autocapitalize="off"/>' +
        '<button id="btn_pickct" class="btn_nexttoinput ui-btn ui-btn-corner-all ui-btn-b noshadow"><img src="' +
        webphone_api.common.GetElementSource() +
        "images/" +
        btnimage +
