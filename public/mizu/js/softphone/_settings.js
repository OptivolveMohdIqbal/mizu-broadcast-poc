...
webphone_api._settings = (function () {
  ...
  function onCreate(event) {
    ...
    try {
      ...
      try {
        window.onerror = function (msg, url, lineNo, columnNo, error) {
          try {
            if (console) {
              msg =
                "ERROR, GlobalErrorHandler catch: " +
                msg +
                " (" +
                error +
                ") url: " +
              ...
              webphone_api.common.PutToDebugLog(
                2,
                "ERROR, GlobalErrorHandler catch: " + msg,
              );
            }
          } catch (errin2) {}
          return false;
      ...
      webphone_api.$(window).resize(function () {
  ...
  function onStart(event) {
    try {
        ...
        //BRANDEND
        {
          ...
          if (webphone_api.common.IsWindowsSoftphone() === true) {
            ...
            if (webphone_api.common.GetParameter("qrcode_login").length > 0) {
              qrcode_login = webphone_api.common.GetParameterInt(
                "qrcode_login",
                0,
              );
            } else {
              qrcode_login = webphone_api.common.GetConfigInt(
                "qrcode_login",
                0,
              );
            }
            ...
            if (qrcode_login == 1) {
              ...
              if (usrtmp.length < 1 || pwdtmp.length < 1) {
                webphone_api
                  .$("#lp_btn_custom")
                  .html(webphone_api.stringres.get("btn_qrcode"));
                webphone_api.$("#lp_btn_custom").show();
              }
      ...
      webphone_api.$("#lp_btn_login").on("click", function () {
        ...
        if (
          !webphone_api.common.isNull(
            document.getElementById("lp_serveraddress"),
          ) &&
          webphone_api
            .$("#lp_serveraddress")
        ...
        if (
          !webphone_api.common.isNull(
            document.getElementById("lp_proxyaddress"),
          ) &&
          webphone_api
            .$("#lp_serveraddress")
            .closest("div.ui-input-text")
          ...
          if (webphone_api.common.isNull(lpproxy)) {
            lpproxy = "";
          }
        ...
        var lpcallerid = "";
        if (
          !webphone_api.common.isNull(document.getElementById("lp_callerid"))
        ) {
          lpcallerid = document.getElementById("lp_callerid").value;
          if (webphone_api.common.isNull(lpcallerid)) {
            lpcallerid = "";
          }
          if (lpcallerid.indexOf("<") >= 0 && lpcallerid.indexOf(">") > 0)
            lpcallerid = webphone_api.common.StrGetBetween(
              lpcallerid,
              "<",
              ">",
            );
          ...
          if (lpcallerid.length > 0)
            webphone_api.common.SaveParameter("username", lpcallerid);
        }
        ...
        if (
          ...
          if (lpusr.indexOf("<") >= 0 && lpusr.indexOf(">") > 0)
            lpusr = webphone_api.common.StrGetBetween(lpusr, "<", ">");
        ...
        for (var ctrl = 0; ctrl < 2; ctrl++) {
          ...
          if (
            ...
            try {
              var handleusernameuri = webphone_api.common.GetParameterInt(
                "handleusernameuri",
                3,
              ); //0: ignore, 1: extract username only on the basic settings, 2: config to serveraddress or proxyaddress (which is empty) in basic settings only, 3: also on the advanced page, 4: config as serveraddress, 5: config as proxyaddress
              var domainpart = webphone_api.common.Trim(
                webphone_api.common.StrGetAfter(mSettValue, "@"),
              );
              if (
                domainpart.length > 0 &&
                handleusernameuri > 0 &&
                (handleusernameuri > 1 || mCurrSettName == "username") &&
                (handleusernameuri >= 4 ||
                  isSettLevelBasic ||
                  mCurrSettName == "username")
              ) {
                if (
                  mCurrSettName == "username" ||
                  webphone_api.common.GetParameterInt(
                    "handlesipusernameuri",
                    -1,
                  ) > 0
                ) {
                  mSettValue = webphone_api.common.Trim(
                    webphone_api.common.StrGetUntill(mSettValue, "@"),
                  );
                }
                //webphone_api.common.SaveParameter('proxyaddress', domainpart);

                var oldserveraddress =
                  webphone_api.common.GetParameter("serveraddress");
                oldserveraddress = webphone_api.common.GetParameter(
                  "serveraddress_orig",
                  oldserveraddress,
                );
                oldserveraddress = webphone_api.common.GetParameter(
                ...
                if (serveraddress_mSettValue.length > 0)
                  oldserveraddress = serveraddress_mSettValue;

                var proxyaddress_mSettValue = lpproxy;
                if (proxyaddress_mSettValue.length > 0)
                  oldproxyaddress = proxyaddress_mSettValue;

                if (
                  handleusernameuri == 2 ||
                  (handleusernameuri < 5 &&
                    (oldserveraddress == domainpart ||
                      oldproxyaddress == domainpart))
                ) {
                  //ignore
                } else {
                  if (
                    handleusernameuri != 6 &&
                    (handleusernameuri == 5 ||
                      serveraddress_mSettValue.length < 1 ||
                      ((handleusernameuri == 3 || handleusernameuri == 4) &&
                        oldproxyaddress.length < 1))
                  ) {
                    if (
                      oldserveraddress.length > 0 &&
                      oldserveraddress != domainpart
                    ) {
                      if (webphone_api.common.CanLog(5)) {
                        webphone_api.common.PutToDebugLog(
                          5,
                          "EVENT, username URI handling AX: " +
                            handleusernameuri.toString() +
                            " " +
                            mSettValue +
                            "@" +
                            domainpart +
                            ...
                            oldserveraddress +
                            "/" +
                            serveraddress_mSettValue +
                            " " +
                            oldproxyaddress +
                            "/" +
                            proxyaddress_mSettValue,
                        );
                      }
                      ...
                      if (webphone_api.common.CanLog(5)) {
                        webphone_api.common.PutToDebugLog(
                          5,
                          "EVENT, username URI handling BX: " +
                            handleusernameuri.toString() +
                            " " +
                            mSettValue +
                            "@" +
                    ...
                    if (
                      ...
                      webphone_api.common.ShowToast(
                        webphone_api.stringres.get("toast_domainasserver"),
                        6000,
                      );
                    }
                  } else if (handleusernameuri == 6) {
                    // || oldproxyaddress.length < 1)
                    ...
                    if (
                      ...
                      if (webphone_api.common.CanLog(5)) {
                        webphone_api.common.PutToDebugLog(
                          5,
                          "EVENT, username URI handling CX: " +
                            handleusernameuri.toString() +
                            " " +
                            mSettValue +
                            "@" +
                      ...
                      webphone_api.common.ShowToast(
                        webphone_api.stringres.get("toast_domainasproxy"),
                        6000,
                      );
                    }
                  }
      ...
      if (
        !webphone_api.common.isNull(document.getElementById("lp_proxyaddress"))
      ) {
        document.getElementById("lp_proxyaddress").placeholder =
      ...
      if (!webphone_api.common.isNull(document.getElementById("lp_callerid"))) {
        document.getElementById("lp_callerid").placeholder =
  ...
  function OfferNativeEngine(popupafterclose) {
    try {
      ...
      webphone_api.$("#pusewebrtc").on("click", function () {
        //-- !!! also set in UserChooseEnginePopupSDK()
      ...
      webphone_api.$("#pusenative").on("click", function () {
        //-- !!! also set in UserChooseEnginePopupSDK()
          ...
          if (webphone_api.common.IsWindowsSoftphone()) {
            setTimeout(function () {
              ...
              webphone_api.startInner(
                webphone_api.common.GetSipusername(true),
  ...
  var settOrderWebphoneWebRTCFlash =
    ...
    aspeak +
    //--        'submenu_screenshare,sscontrol,ssscroll,sstop,ssquality,'+
    "video_fps,aec,agc,volumein,volumeout,playtone";
  ...
  var settOrderWin =
    ...
    "submenu_screenshare,sscontrol,ssscroll,sstop,ssquality," +
    "video_fps,codec,setfinalcodec,alwaysallowlowcodec,aec,denoise,agc,plc,silencesupress,hardwaremedia,volumein,volumeout,playtone,mediaencryption,setqos,codecframecount,doublesendrtp,jittersize,audiobufferlength,speakermode";
  ...
  var basicSettingsCustomized =
    "theme,displayname,ringtone,voicerecording,startonboot,dialerintegration,integrateifwifionly,nativefilterallow,nativefilterblock,cpualwayspartiallock,forcewifi,email,loglevel,startwithos";
  ...
  function PopulateList() {
    ...
    try {
      ...
      var showproxyaddress = webphone_api.common.GetParameterInt(
        "showproxyaddress",
        1,
      );
      ...
      if (
        showproxyaddress == 1 &&
        webphone_api.common.GetParameter("proxyaddress").length > 0 &&
        webphone_api.common.GetParameter("proxyaddress") !=
          webphone_api.common.GetParameter("serveraddress") &&
        webphone_api.common.GetParameter("proxyaddress") !=
          webphone_api.common.GetParameter("serveraddress_user")
      ...
      if (
        ...
        webphone_api.common.GetParameter("username") !=
      ...
      if (ShowLoginPage()) {
        ...
        if (webphone_api.common.ShowServerInput()) {
          ...
          if (
            ...
            webphone_api
              .$("#lp_proxyaddress")
              .val(webphone_api.common.GetParameter("proxyaddress"));
        ...
        var pwdautocomplete = webphone_api.common.GetParameterInt(
          "pwdautocomplete",
          -1,
        );
        if (pwdautocomplete === 0 || pwdautocomplete === 1) {
          try {
            if (pwdautocomplete === 0)
              webphone_api.$("#lp_password").attr("autocomplete", "off");
            else if (pwdautocomplete === 1)
              webphone_api.$("#lp_password").attr("autocomplete", "on");
          } catch (err) {
            webphone_api.common.PutToDebugLogException(
              5,
              "_settings: pwdautocomplete",
              err,
            );
          }
        }
      ...
      if (
        showproxyaddress >= 2 &&
        settOrderTmp.indexOf("serveraddress_user,") >= 0
      ) {
        settOrderTmp = settOrderTmp.replace("proxyaddress,", "");
        settOrderTmp = settOrderTmp.replace(
          "serveraddress_user,",
          "proxyaddress,serveraddress_user,",
        );
      }
      ...
      while (settOrderTmp.length > 0 && loopindex < 5000) {
        ...
        */
        ...
        //BRANDSTART
        if (
          webphone_api.common.GetConfigInt("brandid", -1) === 60
        ) //-- voipmuch
        {
  ...
  function OnListItemClick(id) {
    ...
    try {
      ...
      if (id === "settingitem_chooseengine") {
        webphone_api.common.PutToDebugLog(
          5,
          "EVENT, _settings: chooseengine init",
        );
        var enginelist = webphone_api.common.GetEngineList();
      ...
      if (mSettType === "1") {
        ...
        if (mCurrSettName === "password") {
          ...
          else if (pwdautocomplete == 0)
            autocompleteflag = ' autocomplete="off" ';

          var hidepassword = webphone_api.common.GetParameterInt(
            "hidepassword",
            4,
          );
          ...
          if (
            ...
            inputhtml =
              '<input type="password" id="setting_item_input" name="setting_item" data-theme="a" dir="auto" autocapitalize="off" ' +
              autocompleteflag +
              selectallflag +
              ' ondblclick="ShowHidePwd()"/>';
            inputhtml =
              inputhtml +
              ' <script> function ShowHidePwd() { try{ var pwdinput = document.getElementById("setting_item_input"); if (pwdinput.type === "password") {  pwdinput.type = "text"; } else { pwdinput.type = "password"; } } catch(erra) { } } </script>';
          } else if (
            hidepassword === 3 ||
            (hidepassword === 4 &&
              (webphone_api.common.isNull(mSettValue) || mSettValue.length < 1))
          ) {
            inputhtml =
              '<input type="password" id="setting_item_input" name="setting_item" data-theme="a" dir="auto" autocapitalize="off" ' +
              autocompleteflag +
          ...
          if (
            ...
            if (
              ...
              if (mSettValue.indexOf("<") >= 0 && mSettValue.indexOf(">") > 0) {
                mSettValue = webphone_api.common.StrGetBetween(
                  mSettValue,
                  "<",
                  ">",
                );
              ...
              if (mSettValue.indexOf("@") > 0) {
                try {
                  ...
                  if (
                    ...
                    var serveraddress_value =
                      webphone_api.global.settmap2["serveraddress_user"];
                    serveraddress_value_user = serveraddress_value;
                    if (!webphone_api.common.isNull(serveraddress_value))
                      serveraddress_mSettValue =
                        serveraddress_value[webphone_api.common.SETT_VALUE];
                    if (webphone_api.common.isNull(serveraddress_mSettValue))
                      serveraddress_mSettValue = "";
                    if (serveraddress_mSettValue.length < 1) {
                      var serveraddress_value =
                        webphone_api.global.settmap2["serveraddress_orig"];
                      if (!webphone_api.common.isNull(serveraddress_value))
                        serveraddress_mSettValue =
                          serveraddress_value[webphone_api.common.SETT_VALUE];
                      if (webphone_api.common.isNull(serveraddress_mSettValue))
                      ...
                      if (serveraddress_mSettValue.length < 1) {
                        var serveraddress_value =
                          webphone_api.global.settmap2["serveraddress"];
                        if (!webphone_api.common.isNull(serveraddress_value))
                          serveraddress_mSettValue =
                            serveraddress_value[webphone_api.common.SETT_VALUE];
                        if (
                          webphone_api.common.isNull(serveraddress_mSettValue)
                        )
                          serveraddress_mSettValue = "";
                      }
                    }
                    if (serveraddress_mSettValue.length > 0)
                      oldserveraddress = serveraddress_mSettValue;
                    if (!webphone_api.common.isNull(serveraddress_value_user))
                      serveraddress_value = serveraddress_value_user;

                    var proxyaddress_mSettValue = "";
                    var proxyaddress_value =
                      webphone_api.global.settmap2["proxyaddress"];
                    if (!webphone_api.common.isNull(proxyaddress_value))
                      proxyaddress_mSettValue =
                        proxyaddress_value[webphone_api.common.SETT_VALUE];
                    if (webphone_api.common.isNull(proxyaddress_mSettValue))
                      proxyaddress_mSettValue = "";
                    if (proxyaddress_mSettValue.length > 0)
                      oldproxyaddress = proxyaddress_mSettValue;
                    ...
                    if (
                      ...
                      if (
                        handleusernameuri != 6 &&
                        !webphone_api.common.isNull(serveraddress_value) &&
                        (handleusernameuri == 5 ||
                          serveraddress_mSettValue.length < 1 ||
                          ((handleusernameuri == 3 || handleusernameuri == 4) &&
                            oldproxyaddress.length < 1))
                        ...
                        if (
                          oldserveraddress.length > 0 &&
                          !webphone_api.common.isNull(proxyaddress_value) &&
                          oldserveraddress != domainpart
                        ) {
                          if (webphone_api.common.CanLog(5)) {
                            webphone_api.common.PutToDebugLog(
                              5,
                              "EVENT, username URI handling A: " +
                                handleusernameuri.toString() +
                                " " +
                                mSettValue +
                                "@" +
                          ...
                          proxyaddress_value[webphone_api.common.SETT_VALUE] =
                            oldserveraddress;
                          proxyaddress_value[
                            webphone_api.common.SETT_ISDEFAULT
                          ] = "0";
                          webphone_api.global.settmap["proxyaddress"] =
                            proxyaddress_value;
                          webphone_api.global.settmap2["proxyaddress"] =
                            proxyaddress_value;
                        } else {
                        ...
                        if (
                          ...
                          webphone_api.global.settmap["serveraddress_user"] =
                            serveraddress_value;
                          webphone_api.global.settmap2["serveraddress_user"] =
                            serveraddress_value;
                          webphone_api.common.ShowToast(
            ...
            if (
              mCurrSettName === "password" &&
              textBoxVal.indexOf("***") == 0
            ) {
              return;
            }
      ...
      if (mSettType === "3") {
        if (mCurrSettName === "vcodec" || mCurrSettName === "videocodec") {
          var radiogroup =
            ...
            '<input name="' +
            mCurrSettName +
            '" id="vcodec_optimal" [CHECKED_OPTIMAL] type="checkbox">' +
            '<label for="vcodec_optimal">' +
            webphone_api.stringres.get("vcodec_optimal") +
            "</label>" +
            '<input name="' +
            mCurrSettName +
            '" id="vcodec_h264" [CHECKED_H264] type="checkbox">' +
            '<label for="vcodec_h264">H264</label>' +
            '<input name="' +
            mCurrSettName +
            '" id="vcodec_h265" [CHECKED_H265] type="checkbox">' +
            '<label for="vcodec_h265">H265</label>' +
            '<input name="' +
            mCurrSettName +
            '" id="vcodec_vp8" [CHECKED_VP8] type="checkbox">' +
            '<label for="vcodec_vp8">VP8</label>' +
            '<input name="' +
            mCurrSettName +
            '" id="vcodec_vp9" [CHECKED_VP9] type="checkbox">' +
            '<label for="vcodec_vp9">VP9</label>' +
            '<input name="' +
            mCurrSettName +
            '" id="vcodec_av1" [CHECKED_AV1] type="checkbox">' +
          ...
          if (mSettValue === "-1") // means optimal
          {
            ...
            var ch_h265 = 'checked="checked"';
            if (
              webphone_api.common.GetParameterBool("use_h265", true) === false
            ) {
              ch_h265 = "";
            }
            ...
            var ch_av1 = 'checked="checked"';
            if (
              webphone_api.common.GetParameterBool("use_av1", true) === false
            ) {
              ch_av1 = "";
            }
          ...
          webphone_api.$("#adialog_positive").on("click", function (event) {
            ...
            if (webphone_api.$("#vcodec_optimal").prop("checked")) {
              ...
              var cdh264 = "false";
              if (webphone_api.$("#vcodec_h264").prop("checked")) {
                cdh264 = "true";
              }
              webphone_api.common.SaveParameter("use_h264", cdh264);

              var cdh265 = "false";
              if (webphone_api.$("#vcodec_h265").prop("checked")) {
                cdh265 = "true";
              }
              webphone_api.common.SaveParameter("use_h265", cdh265);

              var cdvp8 = "false";
              if (webphone_api.$("#vcodec_vp8").prop("checked")) {
                cdvp8 = "true";
              }
              webphone_api.common.SaveParameter("use_vp8", cdvp8);

              var cdvp9 = "false";
              if (webphone_api.$("#vcodec_vp9").prop("checked")) {
                cdvp9 = "true";
              }
              webphone_api.common.SaveParameter("use_vp9", cdvp9);

              var cdav1 = "false";
              if (webphone_api.$("#vcodec_av1").prop("checked")) {
                cdav1 = "true";
              }
          ...
          for (var i = 0; i < arrayNames3.length; i++) {
            ...
            if (
              arrayNames3[i] === "SPEEX" ||
              arrayNames3[i] === "SPEEXNB" ||
              arrayNames3[i] === "SPEEX-NB"
            ) {
              for (var j = 0; j < arraySelectedValues3.length; j++) {
            ...
            if (
              arrayNames3[i] === "OpusWB" ||
              arrayNames3[i] === "Opus-WB" ||
              arrayNames3[i] === "Opus"
            ) {
              for (var j = 0; j < arraySelectedValues3.length; j++) {
                if (arraySelectedValues3[j] === "11") {
                  item = item.replace("[CHECKED]", 'checked="checked"');
                  break;
                }
              }
            ...
            if (
              arrayNames3[i] === "G.722" ||
              arrayNames3[i] === "G.722.1" ||
              arrayNames3[i] === "G7221"
            ) {
              for (var j = 0; j < arraySelectedValues3.length; j++) {
                if (arraySelectedValues3[j] === "14") {
                  item = item.replace("[CHECKED]", 'checked="checked"');
                  break;
                }
              }
          ...
          webphone_api.$(":checkbox").on("change", function (event) {
            ...
            if (val === "-1") {
              if (
                webphone_api.$("#" + mCurrSettName + "_0").prop("checked")
      ...
      ) //submenu_sipsettings,submenu_media,submenu_integrate,submenu_calldivert,submenu_general,submenu_integrate
      {
        ...
        if (mCurrSettName === "submenu_sipsettings") {
          ...
          mCurrSettName === "submenu_advanced_artcl"
      ...
      if (mSettType === "8") {
        ...
        if (
          mCurrSettName === "serveraddress_user"
        ) // has different popup (with help button)
        {
          ...
          //BRANDSTART
          if (
            webphone_api.common.GetParameterInt("brandid", -1) === 2
          ) //-- gmsdialergold
          {
            settComment = webphone_api.stringres.get(
              "sett_comment_serveraddress_user_gmsdialer",
          ...
          var template =
            ...
            '<input type="text" ' +
            widthclass +
            ' id="setting_item_input" name="setting_item" data-theme="a" dir="auto" autocapitalize="off"/>' +
            srvaddrpart +
            serverhelp +
            "</div>" +
            '<div data-role="footer" data-theme="b" class="adialog_footer">' +
          ...
          webphone_api.$("#btn_srvhelp").on("click", function (event) {
            ...
            if (webphone_api.global.usestorage == true) {
              ...
              webphone_api.common.AlertDialog(
                ...
                webphone_api.stringres.get("srvaddr_help_cust"),
                null,
                null,
                false,
              );
...
*/
  ...
  var trigerredQ = false; // handle multiple clicks
  function CustomBtn() {
    //favafone newuser or QR code login
    try {
      //BRANDSTART
      if (webphone_api.common.GetConfigInt("brandid", -1) === 50) // favafone
      {
        ...
        webphone_api.common.PutToDebugLog(
          3,
          "EVENT, settings button QRcode login clicked b",
        );
