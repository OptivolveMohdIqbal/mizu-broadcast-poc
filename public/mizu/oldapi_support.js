...
{alert(wp_common.logElement);
  ...
  return {
    ...
    init: function () {
      //	function to initialize all variables, called only on page load
      //##NEW
      ...
      if (document.getElementById("js_not_enabled") != null)
      ...
      try {
        ...
        if (
          ...
          pageUrl !== null &&
          pageUrl.toLowerCase().indexOf("file:///") >= 0
        ) {
          ...
          if (
            archive.indexOf("http://") < 0 &&
            archive.indexOf("https://") < 0
          ) {
            console.log(
              "WARNING, The webphone should be hosted on a webserver. It may not work if launched from your filesystem.",
            );
          }
        }
      } catch (e) {}
      ...
      if (
        lickeyField !== null &&
        typeof lickey !== "undefined" &&
        lickey !== null &&
        lickey.trim().length > 0 &&
        saveSettings !== null &&
        saveSettings.checked === true
      ) {
      ...
      if (
        srvaddrField !== null &&
        typeof srvaddr !== "undefined" &&
        srvaddr !== null &&
        srvaddr.trim().length > 0 &&
        saveSettings !== null &&
        saveSettings.checked === true
      ) {
      ...
      if (wp_api.isMultiLineSkin == true) {
        wp_layout.ManageLines();
      }

      phoneNumberFieldValue = "Enter number";
      if (document.getElementById("PhoneNumber") != null)
        document.getElementById("PhoneNumber").value = phoneNumberFieldValue;
      ...
      if (
        wp_api.haveloginpage !== true &&
        typeof usrReg !== "undefined" &&
        usrReg !== null &&
        usrReg.trim().length > 0 &&
        ((typeof pwdReg !== "undefined" &&
          pwdReg !== null &&
          pwdReg.trim().length > 0) ||
          (typeof md5Reg !== "undefined" &&
            md5Reg !== null &&
            md5Reg.trim().length > 0))
      ) {
      ...
      try {
        ...
        if (document.getElementById("main") !== null) {
          document.getElementById("main").style.width = withTemp + "px";
        }
      } catch (e) {}
      wp_common.isWebpageLoaded = true;
    },
    GetParametersFromCookies: function () {
      // get applet parameters and settings from cookies if exists
      var tempVal = "";

      tempVal = wp_common.readCookie("wp_serveraddress");
      if (
        typeof tempVal !== "undefined" &&
        tempVal !== null &&
        tempVal.trim().length > 0
      ) {
      ...
      if (
        ...
        if (document.getElementById("username_input") !== null) {
          document.getElementById("username_input").value = tempVal.trim();
        }
      ...
      if (
        ...
        if (document.getElementById("password_input") !== null) {
          document.getElementById("password_input").value = tempVal.trim();
        }
      ...
      if (
        ...
        if (
          typeof wp_api.parameters["directserveraddress"] === "undefined" ||
          wp_api.parameters["directserveraddress"] === null
        ) {
          wp_api.parameters["directserveraddress"] = tempVal.trim();
        }
      }
    ...
    GetParametersFromUrl: function () {
      // get applet parameters and settings from URL
      ...
      if (pos > 0) {
        ...
        if (tempVal !== null && tempVal.length > 0) {
          wp_api.parameters["serveraddress"] = tempVal;
        }

        tempVal = wp_common.GetValues(url, "username");
        if (tempVal !== null && tempVal.length > 0) {
          if (document.getElementById("username_input") !== null) {
            document.getElementById("username_input").value = tempVal;
          }
        ...
        if (tempVal !== null && tempVal.length > 0) {
          if (document.getElementById("password_input") !== null) {
            document.getElementById("password_input").value = tempVal;
          }
    ...
    GetValues: function (url, param) {
      ...
      if (param.indexOf("wp_") === 0) {
        param = param.substring(param.indexOf("wp_") + 3, param.length);
      }
    ...
    onLogin: function () {
      //	reset afferent variables when registering: voipRegiste(), voipStatusRegiste()
      hold = false;
      mute = false;
      DTMFcount = 0;
      boolCallLengthTimer = true;
    ...
    onCallFinished: function () {
      //	reset afferent variables when call finished
      hold = false;
      mute = false;
      DTMFcount = 0;
      boolCallLengthTimer = true;
    ...
    getURLParameters: function () {
      // gets parameters from URL in case login page is not used
    ...
    noLoginRegister: function () {
      //	register without login page, showing directly numpad  !!! DEPRECATED
      ...
      try {
        parameters = wp_common.getURLParameters();
      } catch (e) {}
      var srvAddr;
      var usrParam;
      var pssw;
      ...
      if (parameters != null) {
        ...
        if (testParameters == true) {
          for (var j = 0; j < parameters.length; j = j + 2) {
            if (parameters[j] == "serveraddress") {
              srvAddr = parameters[j + 1];
            }
            if (parameters[j] == "username") {
              usrParam = parameters[j + 1];
            }
            if (parameters[j] == "password") {
              pssw = parameters[j + 1];
            }
            if (parameters[j] == "md5") {
              md5Checksum = parameters[j + 1];
            }
            if (parameters[j] == "realm") {
              theRealm = parameters[j + 1];
            }
          }
        }
      } else {
      ...
      if (md5Checksum.length > 2) {
        if (theRealm.length < 2) {
          theRealm = srvAddr;
        }

        applethandle.API_SetCredentialsMD5(
          "" + srvAddr + "",
          "" + usrParam + "",
          "" + md5Checksum + "",
          "" + theRealm + "",
        );
        applethandle.API_Register(
          "" + srvAddr + "",
          "" + usrParam + "",
          "",
          "",
    ...
    pages: function (pageNum) {
      //show login page(1) or Num. Pad page(2)
      currentPage = pageNum;
      ...
      if (pageRegister != null && pageDial != null) {
        if (pageNum == 1) {
          ...
          checkIfPollingTierId = setTimeout(
            "wp_common.isPollingNeeded()",
            2500,
          );
        }
      }
    },
    btnControl: function (id, control) {
      ...
      if (voicemailNr.length > 0 && id == "btn_hold") {
        if (control == 1) {
          document
            .getElementById(id)
            .setAttribute("src", imgPath + "btn_voicemail.jpg");
        }
        if (control == 2) {
          document
            .getElementById(id)
            .setAttribute("src", imgPath + "btn_voicemail_hover.jpg");
        }
      } else {
        var ext = "";
        if (
          (id == "speaker" || id == "logout") &&
          wp_api.isIphoneSkin == false
        ) {
          ext = "png";
        } else {
        ...
        if (control == 1) {
          document
            .getElementById(id)
            .setAttribute("src", imgPath + id + "." + ext);
        }
        if (control == 2) {
          document
            .getElementById(id)
            .setAttribute("src", imgPath + id + "_hover." + ext);
        }
      }
    },
    btnStatusControl: function (id, control, btn) {
      // call, hangup buttons hover and disabled effect
      ...
      if (btn == "call") {
        if (checkCall == true) {
          if (document.getElementById(id) != null) {
            document
              .getElementById(id)
              .setAttribute("src", imgPath + id + "_disabled.jpg");
          }
        } else {
    ...
    onKeyPressEvent: function (event) {
      ...
      if (currentPage == 2 && testFocused == false) {
        ...
        if (pressedKey != -1) {
          wp_common.wp_BtnValue(pressedKey);
        }
      } else {
        if (event.keyCode == 13) {
          //wp_common.wp_Call();
    ...
    wp_BtnValue: function (id) {
      // retrieving values of num pad buttons and sending DTMF if in call
      if (
        document.getElementById("PhoneNumber").value == phoneNumberFieldValue
      ) {
      ...
      if (checkCall == true) {
        if (value != 100 && value != 200) {
          if (!wp_common.initcheck()) {
            alert("Cannot find applet handle (1) !");
            return false;
          }
        ...
        if (value == 100) {
          document.getElementById("PhoneNumber").value = number.slice(
            0,
            number.length - 1,
          );
        } else {
    ...
    displayDTMF: function (strDTMF, delay) {
      // timer, displaying DTMF
      var dtmf = document.getElementById("dtmf");
    ...
    hideDTMF: function () {
      //	hiding DTMF
      checkDTMF = false;
      if (document.getElementById("dtmf") != null)
    ...
    isPollingNeeded: function () {
      if (isAppletLoaded) {
        if (!isWebphoneToJsCalled) {
          if (!wp_common.initcheck()) {
            /*alert('Cannot find applet handle (2) !');*/ return false;
          }
    ...
    webphonetojsCommon: function (varr) {
      ...
      if (isWebphoneToJsCalled == false) {
        webphoneToJsCalledNr++;
      }
      if (webphoneToJsCalledNr > 1) {
        isWebphoneToJsCalled = true;
      }

      wp_common.preProcessNotifications(evNot);
    },
    preProcessNotifications: function (eventNotif) {
      // break notification into lines
      ...
      if (
        eventNotif.length === 0 ||
        (eventNotif.indexOf(notsep) === eventNotif.lastIndexOf(notsep) &&
          eventNotif.indexOf(notsep2) === eventNotif.lastIndexOf(notsep2))
      ) {
        ...
        if (
          eventNotif.indexOf(notsep) !== eventNotif.lastIndexOf(notsep)
        ) // means there is more then one notification, so we have to split them
        {
          var notarray = eventNotif.split(notsep);

          if (!isNull(notarray) && notarray.length > 0) {
            for (var j = 0; j < notarray.length; j++) {
              if (typeof notarray[j] === "undefined" || notarray[j] === null) {
                continue;
              }
    ...
    CleanNotifications: function (not) {
      // remove "WPNOTIFICATION" and "LOG"
      var notsep = ",NEOL \n"; // notification separator (end of line)
      var notsep2 = ",NEOL"; // notification separator (end of line)
    ...
    processNotifications: function (eventNotify) {
      ...
      try {
        if (
          wp_api.ReceiveEvents !== "undefined" &&
          typeof wp_api.ReceiveEvents !== "undefined" &&
          wp_api.ReceiveEvents !== null
        ) {
      ...
      if (posTmp >= 0) {
        var strCredit = eventNotify
          .slice(posTmp + 7, eventNotify.length)
          .trim();
        var onlyCredit = eventNotify
          .slice(0, eventNotify.lastIndexOf(" "))
          .trim();
        var numCredit = -1000000000;

        try {
          numCredit = parseFloat(onlyCredit);
        } catch (e) {}
        ...
        if (onlyCredit !== null && onlyCredit.length > 0) {
          if (numCredit < -999999999) {
            ...
            realCredit = strCredit.replace(
              onlyCredit,
              '<strong style="color:#ff0000">' + onlyCredit + "</strong>",
            );
          } else if (numCredit < lowCredit) {
            realCredit = strCredit.replace(
              onlyCredit,
              "<strong>" + onlyCredit + "</strong>",
            );
          } else {
            realCredit = strCredit.replace(
              onlyCredit,
              '<strong style="color:' +
                wp_api.status_text_color +
                '">' +
                onlyCredit +
                "</strong>",
            );
        ...
        if (
          realCredit == "" &&
          wp_api.parameters["username"] !== null &&
          wp_api.parameters["username"].length > 0
        ) {
          realCredit =
            '<strong style="color:' +
            wp_api.status_text_color +
            '">' +
            wp_api.parameters["username"] +
            "</strong>";
        }
      }
      posTmp = -100;
      ...
      if (eventNotify.indexOf("STATUS") >= 0) {
        ...
        var theStatus = eventNotify.slice(
          eventNotify.indexOf(",") + 1,
          eventNotify.length,
        );
        var statLine = theStatus.slice(0, theStatus.indexOf(","));

        theStatus = theStatus.slice(
          theStatus.indexOf(",") + 1,
          theStatus.length,
        );
      ...
      if (globalStatus.indexOf("EVENT") >= 0) // workaround
      {
        globalStatus = globalStatus
          .substring(globalStatus.lastIndexOf("EVENT") + 6, globalStatus.length)
          .trim();
      }
          ...
          for (var i = 1; i <= wp_api.nrOfLines; i++) {
            if (theLine == "" + i) {
              if (tempStat.indexOf("Ringing") >= 0) {
                var tempCallType = tempStat.substr(
                  tempStat.indexOf(",") + 1,
                  tempStat.length,
                );
                tempCallType = tempCallType.substr(
                  tempCallType.indexOf(",") + 1,
                  tempCallType.length,
                );
                tempCallType = tempCallType.substr(
                  tempCallType.indexOf(",") + 1,
              ...
              if (
                ...
                document.getElementById("btn_line_" + i + "_span").innerHTML =
                  tempStat;
      ...
      ) //	call buttons control
      {
        ...
        if (
          document.getElementById("btn_callhangup") != null
        ) // wp_CallHangup() , when only one button is used for call and hangup
        {
          if ($.browser.msie) {
            $("div#btn_callhangup")
              .children()
              .css("background-color", curr_hangup_button_color);
            $("div#btn_callhangup")
              .children()
              .css(
                "border-color",
                wp_layout.HoverCalc(curr_hangup_button_color, -10),
              );
          } else {
            $("div#btn_callhangup").css(
              "background-color",
          ...
          if (document.getElementById("button_title") != null) {
            document.getElementById("button_title").innerHTML =
              curr_hangup_button_text;
          } // for click2call
        }
      ...
      if (inCallStatus == "Speaking") // used when polling the status
      {
        ...
        ) // wp_CallHangup() , when only one button is used for call and hangup
        {
          ...
          if (document.getElementById("button_title") != null)
            document.getElementById("button_title").innerHTML =
              curr_hangup_button_text; // for click2call
        }
        wp_common.callhangup_isInCall = true;
      }
      if (globalStatus == "CallFinished" || globalStatus == "Call Finished") {
        ...
        ) // wp_CallHangup() , when only one button is used for call and hangup
        {
          if ($.browser.msie) {
            $("div#btn_callhangup")
              .children()
              .css("background-color", wp_layout.curr_call_button_color);
            $("div#btn_callhangup")
              .children()
              .css(
                "border-color",
                wp_layout.HoverCalc(wp_layout.curr_call_button_color, -10),
              );
          } else {
            $("div#btn_callhangup").css(
              "background-color",
              wp_layout.curr_call_button_color,
            );
            $("div#btn_callhangup").css(
              "border-color",
              wp_layout.HoverCalc(wp_layout.curr_call_button_color, -10),
      ...
      if (
        inCallStatus == "InCall" ||
        globalStatus == "CallFinished" ||
        globalStatus == "Call Finished"
      ) {
        if (callButtons != null) callButtons.style.display = "inline-block";
        if (acceptReject != null) acceptReject.style.display = "none";
      }
      ...
      if (
        inCallStatus == "Speaking" ||
        chanelStatus == "Ringing"
      ) //	calculating call length
      {
        if (chanelStatus == "Ringing") {
          callercalled = eventNotify.substring(
            eventNotify.indexOf("Ringing") + 8,
            eventNotify.length,
          );
        ...
        if (checkCall == false) {
          ...
          if (globalStatus == "RegisterFailed") {
            realStatus =
              '<span style="color:#ff0000;">' + globalStatus + "</span>";
          } else {
            realStatus =
              '<span style="color:' +
              wp_api.status_text_color +
      ...
      if (chanelStatus == "Ringing") {
        ...
        if (incomming == "2") {
          ...
          if (document.getElementById("displayed_number") !== null) {
            document.getElementById("displayed_number").innerHTML =
              inCallingNum;
          }

          boolIncommingCall = true;
          wp_common.displayEvent(
            'Incomming call from: <span style="color:' +
              wp_api.status_text_color +
              '; font-weight:bold;">' +
              inCallingNum +
              "</span>",
      ...
      if (globalStatus == "CallFinished") {
        if (callLengthInSec > 2) {
          inCallingField.value = "";
        }
        boolIncommingCall = false;
      }
      if (
        checkCall == true ||
        chanelStatus == "Ringing" ||
        globalStatus == "Incoming..."
      ) //	display credit
      {
        if (
          checkDTMF == false &&
          typeof creditSpan !== "undefined" &&
          creditSpan !== null
        ) {
          if (
            typeof callercalled !== "undefined" &&
            callercalled !== null &&
            callercalled.length > 0
          ) {
      ...
      var eventType = eventNotify.split(",", 2);
      ...
      if (eventName == "EVENT") {
        ...
        if (trialVersionDisplay) {
          return true;
        }

        var evnt = eventNotify.slice(
          eventNotify.indexOf(",") + 1,
          eventNotify.length,
        );
        evnt = evnt.toString();
        evnt = evnt.slice(evnt.indexOf(",") + 1, evnt.length);
        evnt = evnt.substr(0, 1).toUpperCase() + evnt.substr(1);
        evnt = evnt.toString();
        if (evnt.length > 36) {
          evnt = evnt.slice(0, 36);
        }

        if (
          eventType == "EVENT" &&
          eventCredit2 != "EVENT,EVENT,Credit:" &&
          eventDestroy != "EVENT,EVENT, destroying"
        ) {
          wp_common.displayEvent(evnt);
        }
        if (eventType == "WARNING") {
          wp_common.displayEvent(
            '<span style="color:#a30000">' + evnt + "</span>",
          );
        }
        if (eventType == "ERROR") {
          wp_common.displayEvent(
            '<span style="color:#ff0000">' + evnt + "</span>",
          );
        }
        ...
        if (eventCallDuration == "EVENT,EVENT,Call duration:") {
          ...
          if (
            tmpDuration != null &&
            tmpDuration.length > 0 &&
            tmpDuration.indexOf("undefined") < 0
          ) {
            wp_common.displayEvent(
              '<span style="color:' +
                wp_api.status_text_color +
                ';font-weight:bold;">Call duration: ' +
                tmpDuration +
                "</span>",
            );
          }
        ...
        // VOICEMAIL
        var eventVoicem = eventNotify.slice(
          eventNotify.indexOf(",") + 1,
          eventNotify.length,
        );
        eventVoicem = eventVoicem.toString();
        if (eventVoicem.indexOf("MWI") >= 0) {
          var areMessages = eventVoicem.slice(
            eventVoicem.indexOf(",") + 1,
            eventVoicem.length,
          );
          areMessages = areMessages.toString();
          areMessages = areMessages.slice(0, areMessages.indexOf(","));
          areMessages = areMessages.toString();

          if (areMessages == "yes") {
            voicemailNr = eventVoicem.slice(
              eventVoicem.indexOf("MWI,yes") + 8,
              eventVoicem.length,
            );
            voicemailNr = voicemailNr.toString();
            voicemailNr = voicemailNr.slice(0, voicemailNr.indexOf(","));
            voicemailNr = voicemailNr.toString();
      ...
      if (
        ...
        if (dispEvent != null && dispEvent.innerHTML.trim().length <= 0) {
          var durTemp = strTime;
          if (durTemp.indexOf("(Ringing)") >= 0)
            durTemp = durTemp.substring(0, durTemp.indexOf("(Ringing)"));
          wp_common.displayEvent(
            '<span style="color:' +
              wp_api.status_text_color +
              '">Call duration: ' +
    ...
    ChangeHoldToVoicemail: function (change) {
      if (document.getElementById("btn_chat") == null) {
        return;
      }
      ...
      if (change) {
        if (wp_api.isIphoneSkin) {
          document
            .getElementById("btn_hold")
            .setAttribute("src", imgPath + "btn_voicemail.jpg");
          document.getElementById("btn_hold").title = "Voicemail";
        } else {
          document.getElementById("btn_hold").title = "Voicemail";
          document
            .getElementById("btn_hold_img")
            .setAttribute("src", imgPath + "voicemail.png");
        ...
        if (wp_api.isIphoneSkin) {
          document
            .getElementById("btn_hold")
            .setAttribute("src", imgPath + "btn_hold.jpg");
          document.getElementById("btn_hold").title = "Call Hold";
        } else {
          document.getElementById("btn_hold").title = "Call Hold";
          document
            .getElementById("btn_hold_img")
            .setAttribute("src", imgPath + "hold.png");
          document.getElementById("btn_hold_span").innerHTML = "Hold";
        }
      }
    },
    displaystatus: function (statustr) {
      //	display status massages
      if (webphoneStatus == null) {
        try {
          webphoneStatus = document.getElementById("status");
        } catch (e) {}
      }
      try {
        if (
          typeof statustr === "undefined" ||
          statustr === null ||
          statustr.indexOf("Outband") >= 0 ||
          statustr.indexOf("Deletable") >= 0
        ) {
          return;
        }
        ...
        if (
          ...
          // customized for aoudjehane
          if (document.getElementById("status2") !== null) {
            document.getElementById("status2").innerHTML = statustr;
          }
        }
      } catch (e) {}
    },
    displayEvent: function (evStr) {
      //	display event massages
      var displayEvent = document.getElementById("displayEvent");
      if (
        displayEvent !== null &&
        typeof evStr !== "undefined" &&
        evStr !== null
      ) {
        ...
        if (pos > 0) {
          evStr = evStr.substring(pos + 1, evStr.length);
        }
    ...
    dispCallLength: function () {
      //	display call length
      try {
        var currTime = new Date();
        var currentTime = currTime.getTime();
        var callLengthSec = parseInt((currentTime - callStartTime) / 1000);
        callLengthInSec = callLengthSec;
      ...
      strTime += "</span>";
      if (timerDisplayRinging == true) {
        wp_common.displaystatus(
          '<span style="color: #444444">' + strTime + "</span>",
        );
      } else {
    ...
    initcheck: function () {
      ...
      if (
        strApplethandle == null ||
        strApplethandle == "" ||
        (strApplethandle.toLowerCase().indexOf("[object") >= 0 &&
          strApplethandle.toLowerCase().indexOf("[object") < 5)
      ) {
        applethandle = null;
        return false;
      } else {
    ...
    Encrypt: function (str, key) {
      ...
      if (mult < 2) {
        alert(
          "Algorithm cannot find a suitable hash. Please choose a different key. \nPossible considerations are to choose a more complex or longer key.",
        );
        return null;
      }
    ...
    wp_LoadApplet: function () {
      ...
      var appletString = function () {
        ...
        for (a in wp_api.parameters) {
          ...
          param.push(
            "<param name='" + a + "' value='" + wp_api.parameters[a] + "'/>",
          );
        }

        // error message and download link to show if java is not available
        var errMsg =
          '<div id="javaerrormsg" style="clear:both; padding:5px; display:' +
          errmsgVisibility +
          '; background:#fff; color:#f00; font-size:14px; font-weight:bold;">You must enable java or install it<br />if not already installed from <a href="https://java.com/en/download/index.jsp" target="_blank">here</a></div>';

        var errMsg2 =
          '<b>You must enable java or install from <a href="https://www.java.com/en/download/index.jsp"> here </a></b>';
      ...
      var sp = $("<div/>");
      sp.attr("id", "wpappletconatiner")
        .attr(
          "style",
          "z-index:10000; position:absolute; float:left; top:" +
            positionTop +
            "px; left:" +
            positionLeft +
            "px;",
    ...
    wp_LoadAppletAndRegister: function () {
      ...
      if (
        usrField !== null &&
        typeof usrField.value !== "undefined" &&
        usrField.value !== null &&
        usrField.value.length > 0
      ) {
      ...
      if (
        pwdField !== null &&
        typeof pwdField.value !== "undefined" &&
        pwdField.value !== null &&
        pwdField.value.length > 0
      ) {
      ...
      if (
        srvAddr !== null &&
        typeof srvAddr.value !== "undefined" &&
        srvAddr.value !== null &&
        srvAddr.value.length > 0
      ) {
    ...
    wp_Register: function (user, pwd) {
      // in case you don't want to use wp_LoadAppletAndRegister
      wp_common.onLogin();
    ...
    waitForWebphone: function () {
      //	delaying register, when login page not used  !!! DEPRECATED
      ...
      if (appletLoadedBool) // if applet was successfully lodaded
      {
        ...
        wp_common.createCookie(
          "MZwebPhoneIsFirstStart",
          "false",
          "" + cookieExpire + "",
        );
    ...
    voipDelayedRegister: function () {
      //	making delay for API_ServerInit()
      if (!wp_common.initcheck()) {
        alert("Cannot find applet handle (3) !");
        return false;
      }
      ...
      if (
        wp_api.parameters["username"] === "undefined" ||
        wp_api.parameters["username"] === null ||
        wp_api.parameters["username"].length < 1 ||
        wp_api.parameters["password"] === "undefined" ||
        wp_api.parameters["password"] === null ||
        wp_api.parameters["password"].length < 0
      ) {
        ...
        if (licKey != null) {
          ...
          if (licKey.value == "") {
            try {
              document.getElementById("login_page_error_messages").innerHTML =
                "Invalid license key";
            } catch (e) {}
            licKey.focus();
            return false;
...
//##			isRegistered = applethandle.API_SetCredentials(wp_api.parameters['serveraddress'], wp_api.parameters['username'], wp_api.parameters['password'], wp_api.parameters['username'], wp_api.parameters['username']);
    ...
    voipStatusRegister: function () {
      //	register if double click on status and not in call
      return true;
      /*    
    ...
    reRegister: function () {
      // !!!!DEPRECATED calls API_Register() every "reRegisterTime" miliseconds
      return true;
      /*
    ...
    wp_Call: function (calledNr) {
      ...
      if (
        typeof calledNr !== "undefined" &&
        calledNr !== null &&
        calledNr.length > 0 &&
        calledNr != phoneNumberFieldValue
      ) {
        // customized for aoudjehane
        if (document.getElementById("displayed_number") !== null) {
          document.getElementById("displayed_number").innerHTML = calledNr;
        }
    ...
    wp_CallHangup: function () {
      if (wp_common.callhangup_isInCall) {
        ...
        wp_common.displayEvent(
          '<span style="color:#26b14a;">Starting...</span>',
        );
        var boolCall = false;
        var calledNumber = document.getElementById("PhoneNumber");
        if (
          calledNumber.value != "" &&
          calledNumber.value != phoneNumberFieldValue
        ) {
          boolCall = applethandle.API_Call(
            wp_common.currLine,
            "" + calledNumber.value + "",
          );
        ...
        if (boolCall == false) {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">Enter destination first.</span>',
          );
        } else {
          wp_common.callhangup_isInCall = true;
        }
    ...
    wp_RegisterCallHangup: function () {
      ...
      if (!wp_common.initcheck()) {
        alert("Cannot find applet handle (11) !");
        return false;
      }
    ...
    ClickToCallDelayd: function () {
      var boolCall = false;
      if (
        wp_api.destination_number != null &&
        wp_api.destination_number.length > 1
      ) {
        boolCall = applethandle.API_Call(
          wp_common.currLine,
          "" + wp_api.destination_number + "",
        );
      ...
      if (boolCall == false) {
        wp_common.displayEvent(
          '<span style="color:#ff0000;">Destination number is not defined.</span>',
        );
      } else {
        wp_common.callhangup_isInCall = true;
      }
    ...
    wp_Accept: function () {
      ...
      if (boolAccept == false) {
        wp_common.displayEvent(
          '<span style="color:#ff0000;">Accept Failed</span>',
        );
      }
      return boolAccept;
    },
    ...
    wp_Hold: function (holdParam) {
      ...
      try {
        if (typeof holdParam !== "undefined" && holdParam !== null) {
          holdValue = holdParam;
        }
      } catch (e) {}
      ...
      if (boolHold == true) {
        ...
        if (checkCall == true) {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">Hold failed</span>',
          );
        } else {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">No call in progress</span>',
          );
        }
      }
      return boolHold;
    ...
    wp_Voicemail: function (vmNumber) {
      ...
      try {
        if (typeof vmNumber !== "undefined" && vmNumber !== null) {
          vmnrTemp = vmNumber;
        }
      } catch (e) {}
    ...
    voipRedial: function () {
      ...
      if (boolRedial == false) {
        wp_common.displayEvent(
          '<span style="color:#ff0000;">No previous call found</span>',
        );
      }
      return true;
    },
    ...
    wp_CallTransfer: function () {
      ...
      if (boolTransfer == false) {
        if (checkCall == true) {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">Transfer failed</span>',
          );
        } else {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">No call in progress</span>',
    ...
    wp_Mute: function (muteParam) {
      ...
      try {
        if (typeof muteParam !== "undefined" && muteParam !== null) {
          muteValue = muteParam;
        }
      } catch (e) {}
      ...
      if (boolMute == true) {
        ...
        if (checkCall == true) {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">Mute failed</span>',
          );
        } else {
          wp_common.displayEvent(
            '<span style="color:#ff0000;">No call in progress</span>',
    ...
    wp_Redial: function () {
      if (checkCall == true) {
        wp_common.displayEvent(
          '<span style="color:#ff0000;">Call in progress</span>',
        );
        return false;
      } else {
        if (redial != null && redial.length > 0) {
          ...
          wp_common.displayEvent(
            '<span style="color:#ff0000;">No number to redial</span>',
          );
        }
      }
      return false;
